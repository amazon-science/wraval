...
def route_completion(settings, queries):
    ...
    if settings.endpoint_type == "bedrock":
        ...
        prompts = [format_prompt(text, tone_prompt, type="bedrock") for text in queries]
        print(f"Sample prompt:\n{prompts[0]}")
        m = settings.model.format(aws_account=aws_account)
        client = boto3.client(service_name="bedrock-runtime", region_name=settings.region)
        outputs = batch_get_completions(settings.model, client, prompts, [master_sys_prompt] * n)
