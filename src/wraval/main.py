...
def generate(
    ...
    tone: ToneType = typer.Option(ToneType.ALL, "--type", "-t", help="Type of dataset to generate"),
    upload_s3: bool = typer.Option(False, "--upload-s3", help="Upload generated datasets to S3"),
    custom_prompts: bool = typer.Option(False, "--custom-prompts", help="Load custom prompts from a prompt folder"),
    local_tokenizer_path: Optional[str] = typer.Option(None, "--local-tokenizer-path", help="Path to local tokenizer")
):
...
def inference(
    model: str = typer.Option("haiku-3", "--model", "-m", help="Model to use"),
    tone: ToneType = typer.Option(ToneType.ALL, "--type", "-t", help="Type of dataset to process"),
...
@app.command()
def llm_judge(
    model: str = typer.Option("haiku-3", "--model", "-m", help="Model to use for judging"),
    tone: ToneType = typer.Option(ToneType.ALL, "--type", "-t", help="Type of dataset to judge"),
    upload_s3: bool = typer.Option(False, "--upload-s3", help="Upload results to S3"),
    custom_prompts: bool = typer.Option(False, "--custom-prompts", help="Load custom prompts from a prompt folder"),
    local_tokenizer_path: Optional[str] = typer.Option(None, "--local-tokenizer-path", help="Path to local tokenizer")
):
...
def show_examples(
    model: str = typer.Option("haiku-3", "--model", "-m", help="Model to use"),
    tone: ToneType = typer.Option(ToneType.ALL, "--type", "-t", help="Type of dataset to show examples for"),
    n_examples: int = typer.Option(10, "--n-examples", "-n", help="Number of examples to show per tone-model combination"),
    custom_prompts: bool = typer.Option(False, "--custom-prompts", help="Load custom prompts from a prompt folder"),
    local_tokenizer_path: Optional[str] = typer.Option(None, "--local-tokenizer-path", help="Path to local tokenizer")
):
...
def deploy(
    ...
    cleanup_endpoints: bool = typer.Option(False, "--cleanup-endpoints", help="Cleanup endpoints before deploying new models"),
    custom_prompts: bool = typer.Option(False, "--custom-prompts", help="Load custom prompts from a prompt folder"),
    local_tokenizer_path: Optional[str] = typer.Option(None, "--local-tokenizer-path", help="Path to local tokenizer")
):
