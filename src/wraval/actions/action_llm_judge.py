...
def judge(
    ...
    for tone, inf_model in product(tones, inf_models):
        ...
        temp_results = process_tone_data(
            settings, results[mask], tone, model_name, client, tone_rubrics
        )
...
def rewrite_judge(
    model_id: str, bedrock_client: boto3.client, queries: List[str], answers: List[str]
) -> pd.DataFrame:
    ...
    results["rewrite_score"] = batch_get_bedrock_completions(
        model_id, bedrock_client, prompts, max_concurrent=len(prompts)
    )
    return results
